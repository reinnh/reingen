...
const Navbar = () => {
    ...
    <ul className={`list-none ${isSecondary ? 'flex sm:hidden' : 'hidden sm:flex'} flex-row gap-6`}>
      {navLinks.map((link) => (
        <li
          key={link.id}
          className={`${
            active === link.title ? 'text-white' : isSecondary ? 'text-white' : 'text-white'
  ...
  return (
    <>
      <nav
        className={`${styles.paddingX} w-full flex items-center py-3 fixed top-0 z-20 bg-gray-950`}
      >
        <div className="w-full flex justify-between items-center container mx-auto">
          ...
          >
           ...
           <img src={me} alt="logo" className=" rounded-lg" />
           </div>
